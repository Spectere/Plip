...
void PlipIo::CloseFile(std::fstream& file) {
    file.close();
}
...
std::vector<char> PlipIo::ReadFile(const std::string& path, const int offset, const uintmax_t size) {
    auto file = LoadFile(path);
    const auto contents = ReadFile(file, offset, size);
    file.close();
    return contents;
}

std::vector<char> PlipIo::ReadSequential(std::fstream& file, const uintmax_t size) {
    std::vector<char> data(size);
    file.read(data.data(), static_cast<long>(data.size()));
    return data;
}
