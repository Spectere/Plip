/* SharpLr35902.Ld8.cpp
 *
 * Tests the CPU core's ability to load 8-bit values.
 */

#include "catch2/catch_test_macros.hpp"

#include "SharpLr35902Common.h"

TEST("LD <BC>, A", "LD-BC,A") { // 0x02
    cpu->SetBc(0x200);
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x02);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD B, imm8", "LD-B,imm8") { // 0x06
    LoadData(0x00, { 0x06, 0x34 });

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD A, <BC>", "LD-A,BC") { // 0x0A
    cpu->SetBc(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x0A);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD C, imm8", "LD-C,imm8") { // 0x0E
    LoadData(0x00, { 0x0E, 0x34 });

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD <DE>, A", "LD-DE,A") { // 0x12
    cpu->SetDe(0x200);
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x12);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD D, imm8", "LD-D,imm8") { // 0x16
    LoadData(0x00, { 0x16, 0x34 });

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD A, <DE>", "LD-A,DE") { // 0x1A
    cpu->SetDe(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x1A);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD E, imm8", "LD-E,imm8") { // 0x1E
    LoadData(0x00, { 0x1E, 0x34 });

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD <HLI>, A", "LD-HLI,A") { // 0x22
    cpu->SetHl(0x200);
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x22);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
    CHECK(cpu->GetHl() == 0x201);
}

TEST("LD H, imm8", "LD-H,imm8") { // 0x26
    LoadData(0x00, { 0x26, 0x34 });

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD A, <HLI>", "LD-A,HLI") { // 0x2A
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x2A);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
    CHECK(cpu->GetHl() == 0x201);
}

TEST("LD L, imm8", "LD-L,imm8") { // 0x2E
    LoadData(0x00, { 0x2E, 0x34 });

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD <HLD>, A", "LD-HLD,A") { // 0x32
    cpu->SetHl(0x200);
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x32);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
    CHECK(cpu->GetHl() == 0x1FF);
}

TEST("LD <HL>, imm8", "LD-HL,imm8") { // 0x36
    cpu->SetHl(0x200);
    LoadData(0x00, { 0x36, 0x34 });

    EXECUTE(3);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD A, <HLD>", "LD-A,HLD") { // 0x3A
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x3A);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
    CHECK(cpu->GetHl() == 0x1FF);
}

TEST("LD A, imm8", "LD-A,imm8") { // 0x3E
    LoadData(0x00, { 0x3E, 0x34 });

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD B, B", "LD-B,B") {  // 0x40
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    LoadData(0x00, 0x40);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD B, C", "LD-B,C") {  // 0x41
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    LoadData(0x00, 0x41);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD B, D", "LD-B,D") {  // 0x42
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    LoadData(0x00, 0x42);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD B, E", "LD-B,E") {  // 0x43
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    LoadData(0x00, 0x43);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD B, H", "LD-B,H") {  // 0x44
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexH, 0x34);
    LoadData(0x00, 0x44);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD B, L", "LD-B,L") {  // 0x45
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexL, 0x34);
    LoadData(0x00, 0x45);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD B, <HL>", "LD-B,memHL") {  // 0x46
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x46);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD B, A", "LD-B,A") {  // 0x47
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x47);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexB) == 0x34);
}

TEST("LD C, B", "LD-C,B") {  // 0x48
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    LoadData(0x00, 0x48);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD C, C", "LD-C,C") {  // 0x49
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    LoadData(0x00, 0x49);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD C, D", "LD-C,D") {  // 0x4A
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    LoadData(0x00, 0x4A);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD C, E", "LD-C,E") {  // 0x4B
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    LoadData(0x00, 0x4B);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD C, H", "LD-C,H") {  // 0x4C
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexH, 0x34);
    LoadData(0x00, 0x4C);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD C, L", "LD-C,L") {  // 0x4D
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexL, 0x34);
    LoadData(0x00, 0x4D);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD C, <HL>", "LD-C,memHL") {  // 0x4E
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x4E);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD C, A", "LD-C,A") {  // 0x4F
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x4F);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexC) == 0x34);
}

TEST("LD D, B", "LD-D,B") {  // 0x50
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    LoadData(0x00, 0x50);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD D, C", "LD-D,C") {  // 0x51
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    LoadData(0x00, 0x51);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD D, D", "LD-D,D") {  // 0x52
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    LoadData(0x00, 0x52);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD D, E", "LD-D,E") {  // 0x53
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    LoadData(0x00, 0x53);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD D, H", "LD-D,H") {  // 0x54
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexH, 0x34);
    LoadData(0x00, 0x54);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD D, L", "LD-D,L") {  // 0x55
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexL, 0x34);
    LoadData(0x00, 0x55);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD D, <HL>", "LD-D,memHL") {  // 0x56
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x56);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD D, A", "LD-D,A") {  // 0x57
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x57);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexD) == 0x34);
}

TEST("LD E, B", "LD-E,B") {  // 0x58
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    LoadData(0x00, 0x58);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD E, C", "LD-E,C") {  // 0x59
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    LoadData(0x00, 0x59);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD E, D", "LD-E,D") {  // 0x5A
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    LoadData(0x00, 0x5A);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD E, E", "LD-E,E") {  // 0x5B
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    LoadData(0x00, 0x5B);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD E, H", "LD-E,H") {  // 0x5C
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexH, 0x34);
    LoadData(0x00, 0x5C);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD E, L", "LD-E,L") {  // 0x5D
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexL, 0x34);
    LoadData(0x00, 0x5D);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD E, <HL>", "LD-E,memHL") {  // 0x5E
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x5E);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD E, A", "LD-E,A") {  // 0x5F
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x5F);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexE) == 0x34);
}

TEST("LD H, B", "LD-H,B") {  // 0x60
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    LoadData(0x00, 0x60);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD H, C", "LD-H,C") {  // 0x61
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    LoadData(0x00, 0x61);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD H, D", "LD-H,D") {  // 0x62
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    LoadData(0x00, 0x62);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD H, E", "LD-H,E") {  // 0x63
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    LoadData(0x00, 0x63);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD H, H", "LD-H,H") {  // 0x64
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexH, 0x34);
    LoadData(0x00, 0x64);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD H, L", "LD-H,L") {  // 0x65
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexL, 0x34);
    LoadData(0x00, 0x65);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD H, <HL>", "LD-H,memHL") {  // 0x66
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x66);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD H, A", "LD-H,A") {  // 0x67
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x67);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexH) == 0x34);
}

TEST("LD L, B", "LD-L,B") {  // 0x68
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    LoadData(0x00, 0x68);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD L, C", "LD-L,C") {  // 0x69
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    LoadData(0x00, 0x69);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD L, D", "LD-L,D") {  // 0x6A
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    LoadData(0x00, 0x6A);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD L, E", "LD-L,E") {  // 0x6B
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    LoadData(0x00, 0x6B);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD L, H", "LD-L,H") {  // 0x6C
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexH, 0x34);
    LoadData(0x00, 0x6C);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD L, L", "LD-L,L") {  // 0x6D
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexL, 0x34);
    LoadData(0x00, 0x6D);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD L, <HL>", "LD-L,memHL") {  // 0x6E
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x6E);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD L, A", "LD-L,A") {  // 0x6F
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x6F);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexL) == 0x34);
}

TEST("LD <HL>, B", "LD-HL,B") {  // 0x70
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    cpu->SetHl(0x200);
    LoadData(0x00, 0x70);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD <HL>, C", "LD-HL,C") {  // 0x71
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    cpu->SetHl(0x200);
    LoadData(0x00, 0x71);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD <HL>, D", "LD-HL,D") {  // 0x72
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    cpu->SetHl(0x200);
    LoadData(0x00, 0x72);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD <HL>, E", "LD-HL,E") {  // 0x73
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    cpu->SetHl(0x200);
    LoadData(0x00, 0x73);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD <HL>, H", "LD-HL,H") {  // 0x74
    cpu->SetHl(0x0405);
    LoadData(0x00, 0x74);

    EXECUTE(2);
    CHECK(memory->GetByte(0x0405) == 0x04);
}

TEST("LD <HL>, L", "LD-HL,L") {  // 0x75
    cpu->SetHl(0x0405);
    LoadData(0x00, 0x75);

    EXECUTE(2);
    CHECK(memory->GetByte(0x0405) == 0x05);
}

TEST("LD <HL>, A", "LD-HL,A") {  // 0x77
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    cpu->SetHl(0x200);
    LoadData(0x00, 0x77);

    EXECUTE(2);
    CHECK(memory->GetByte(0x200) == 0x34);
}

TEST("LD A, B", "LD-A,B") {  // 0x78
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexB, 0x34);
    LoadData(0x00, 0x78);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, C", "LD-A,C") {  // 0x79
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x34);
    LoadData(0x00, 0x79);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, D", "LD-A,D") {  // 0x7A
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexD, 0x34);
    LoadData(0x00, 0x7A);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, E", "LD-A,E") {  // 0x7B
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexE, 0x34);
    LoadData(0x00, 0x7B);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, H", "LD-A,H") {  // 0x7C
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexH, 0x34);
    LoadData(0x00, 0x7C);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, L", "LD-A,L") {  // 0x7D
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexL, 0x34);
    LoadData(0x00, 0x7D);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, <HL>", "LD-A,memHL") {  // 0x7E
    cpu->SetHl(0x200);
    memory->SetByte(0x200, 0x34);
    LoadData(0x00, 0x7E);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, A", "LD-A,A") {  // 0x7F
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, 0x7F);

    EXECUTE(1);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LDH <imm8>, A", "LD-imm8,A") {  // 0xE0
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, { 0xE0, 0x12 });

    EXECUTE(3);
    CHECK(memory->GetByte(0xFF12) == 0x34);
}

TEST("LDH <C>, A", "LDH-C,A") {  // 0xE2
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x12);
    LoadData(0x00, 0xE2);

    EXECUTE(2);
    CHECK(memory->GetByte(0xFF12) == 0x34);
}

TEST("LD <a16>, A", "LD-a16,A") {  // 0xEA
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexA, 0x34);
    LoadData(0x00, { 0xEA, 0x89, 0x67 });

    EXECUTE(4);
    CHECK(memory->GetByte(0x6789) == 0x34);
}

TEST("LDH A, <imm8>", "LD-A,imm8") {  // 0xF0
    memory->SetByte(0xFF12, 0x34);
    LoadData(0x00, { 0xF0, 0x12 });

    EXECUTE(3);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LDH A, <C>", "LD-A,C") {  // 0xF2
    cpu->SetRegister8ByIndex(MockSharpLr35902::RegIndexC, 0x12);
    memory->SetByte(0xFF12, 0x34);
    LoadData(0x00, 0xF2);

    EXECUTE(2);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}

TEST("LD A, <imm16>", "LD-A,imm16") {  // 0xFA
    memory->SetByte(0x6789, 0x34);
    LoadData(0x00, { 0xFA, 0x89, 0x67 });

    EXECUTE(4);
    CHECK(cpu->GetRegister8ByIndex(MockSharpLr35902::RegIndexA) == 0x34);
}
